...
#
# class User(UserMixin, db.Model):
#     __tablename__ = "new_user_registration"
#     id = db.Column(db.Integer, primary_key=True)
#     name = db.Column(db.String(150), nullable=False)
#     email = db.Column(db.String(150), unique=True, nullable=False)
#     password = db.Column(db.String(150), unique=True, nullable=False)
...
def register():
    ...
    if form.validate_on_submit():
        if user_email:
            flash("You've already singed up with that email,login instead!")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Chack user password hash against hash password
        if not user:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return render_template("post.html", form=form, post=requested_post, current_user=current_user, gravatar=gravatar)
